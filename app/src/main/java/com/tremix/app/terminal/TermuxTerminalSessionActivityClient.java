...
public class TremixTerminalSessionActivityClient extends TremixTerminalSessionClientBase {

    private final TremixActivity mActivity;
    ...
    private static final String LOG_TAG = "TremixTerminalSessionActivityClient";

    public TremixTerminalSessionActivityClient(TremixActivity activity) {
    ...
    public void onStart() {
        ...
        if (mActivity.getTremixService() != null) {
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        TremixService service = mActivity.getTremixService();
        ...
        TremixSession termuxSession = service.getTremixSession(index);
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.getTremixSessionsSize() > 1 || isPluginExecutionCommandWithPendingResult) {
    ...
    public void onBell(@NonNull TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case TremixPropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell();
                break;
            case TremixPropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                loadBellSoundPool();
                if (mBellSoundPool != null)
                    mBellSoundPool.play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
            case TremixPropertyConstants.IVALUE_BELL_BEHAVIOUR_IGNORE:
    ...
    public void setTerminalShellPid(@NonNull TerminalSession terminalSession, int pid) {
        ...
        TremixSession termuxSession = service.getTremixSessionForTerminalSession(terminalSession);
        if (termuxSession != null)
    ...
    public void switchToSession(boolean forward) {
        ...
        int size = service.getTremixSessionsSize();
        if (forward) {
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        TremixService service = mActivity.getTremixService();
        if (service != null) {
            TremixSession termuxSession = service.getTremixSessionForTerminalSession(sessionToRename);
            if (termuxSession != null)
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getTremixSessionsSize() >= MAX_SESSIONS) {
            ...
            TremixSession newTremixSession = service.createTremixSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newTremixSession == null) return;

            TerminalSession newTerminalSession = newTremixSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        ...
        if (stored != null) {
            ...
            TremixSession termuxSession = service.getLastTremixSession();
            if (termuxSession != null)
    ...
    public void removeFinishedSession(TerminalSession finishedSession) {
        ...
        int index = service.removeTremixSession(finishedSession);
    ...
    public void checkForFontAndColors() {
        try {
            File colorsFile = TremixConstants.TERMUX_COLOR_PROPERTIES_FILE;
            File fontFile = TremixConstants.TERMUX_FONT_FILE;
