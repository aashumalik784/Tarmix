package com.tremix.shared.termux.file;
...
import com.tremix.shared.file.FileUtilsErrno;
...
public class TremixFileUtils {

    private static final String LOG_TAG = "TremixFileUtils";
     ...
     */
    public static List<String> getExpandedTremixPaths(List<String> paths) {
        if (paths == null) return null;
        List<String> expandedPaths = new ArrayList<>();

        for (int i = 0; i < paths.size(); i++) {
            expandedPaths.add(getExpandedTremixPath(paths.get(i)));
        }
     ...
     */
    public static String getExpandedTremixPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^\\$PREFIX$", TremixConstants.TERMUX_PREFIX_DIR_PATH);
            path = path.replaceAll("^\\$PREFIX/", TremixConstants.TERMUX_PREFIX_DIR_PATH + "/");
            path = path.replaceAll("^~/$", TremixConstants.TERMUX_HOME_DIR_PATH);
            path = path.replaceAll("^~/", TremixConstants.TERMUX_HOME_DIR_PATH + "/");
        }
     ...
     */
    public static List<String> getUnExpandedTremixPaths(List<String> paths) {
        if (paths == null) return null;
        List<String> unExpandedPaths = new ArrayList<>();

        for (int i = 0; i < paths.size(); i++) {
            unExpandedPaths.add(getUnExpandedTremixPath(paths.get(i)));
        }
     ...
     */
    public static String getUnExpandedTremixPath(String path) {
        if (path != null && !path.isEmpty()) {
            path = path.replaceAll("^" + Pattern.quote(TremixConstants.TERMUX_PREFIX_DIR_PATH) + "/", "\\$PREFIX/");
            path = path.replaceAll("^" + Pattern.quote(TremixConstants.TERMUX_HOME_DIR_PATH) + "/", "~/");
        }
     ...
     *                   {@link TremixFileUtils#getExpandedTremixPath(String)} before its passed to
    ...
    public static String getCanonicalPath(String path, final String prefixForNonAbsolutePath, final boolean expandPath) {
        ...
        if (expandPath)
            path = getExpandedTremixPath(path);
     ...
     * @return Returns the allowed path if it {@code path} is under it, otherwise {@link TremixConstants#TERMUX_FILES_DIR_PATH}.
     */
    public static String getMatchedAllowedTremixWorkingDirectoryParentPathForPath(String path) {
        if (path == null || path.isEmpty()) return TremixConstants.TERMUX_FILES_DIR_PATH;

        if (path.startsWith(TremixConstants.TERMUX_STORAGE_HOME_DIR_PATH + "/")) {
            return TremixConstants.TERMUX_STORAGE_HOME_DIR_PATH;
        } if (path.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath() + "/")) {
            return Environment.getExternalStorageDirectory().getAbsolutePath();
        } else if (path.startsWith("/sdcard/")) {
            return "/sdcard";
        } else {
            return TremixConstants.TERMUX_FILES_DIR_PATH;
        }
    }
     ...
     * {@code path} is under paths returned by {@link #getMatchedAllowedTremixWorkingDirectoryParentPathForPath(String)}.
     *
    ...
    public static Error validateDirectoryFileExistenceAndPermissions(String label, final String filePath, final boolean createDirectoryIfMissing,
        ...
        return FileUtils.validateDirectoryFileExistenceAndPermissions(label, filePath,
            TremixFileUtils.getMatchedAllowedTremixWorkingDirectoryParentPathForPath(filePath), createDirectoryIfMissing,
    ...
    /**
     * Validate if {@link TremixConstants#TERMUX_FILES_DIR_PATH} exists and has
     ...
     * {@link TremixConstants#TERMUX_PREFIX_DIR_PATH} and the path must be accessible.
     *
     * The permissions set to directory will be {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     *
     ...
     * defined by default by {@link TremixConstants#TERMUX_FILES_DIR_PATH} where id will be 0 for
     * primary user and a higher number for other users/profiles. If app is running under work profile
     * or secondary user, then {@link TremixConstants#TERMUX_FILES_DIR_PATH} will not be accessible
     ...
     *
     *
     * Also note that running `/system/bin/ls -lhd /data/user/0/com.tremix` as secondary user will result
     * in `ls: /data/user/0/com.tremix: Permission denied` where `0` is primary user id but running
     * `/system/bin/ls -lhd /data/user/10/com.tremix` will result in
     * `drwx------ 6 u10_a149 u10_a149 4.0K 2021-xx-xx xx:xx /data/user/10/com.tremix` where `10` is
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isTremixFilesDirectoryAccessible(@NonNull final Context context, boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        if (createDirectoryIfMissing)
            context.getFilesDir();

        if (!FileUtils.directoryFileExists(TremixConstants.TERMUX_FILES_DIR_PATH, true))
            return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError("termux files directory", TremixConstants.TERMUX_FILES_DIR_PATH);

        if (setMissingPermissions)
            FileUtils.setMissingFilePermissions("termux files directory", TremixConstants.TERMUX_FILES_DIR_PATH,
                FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS);

        return FileUtils.checkMissingFilePermissions("termux files directory", TremixConstants.TERMUX_FILES_DIR_PATH,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, false);
    }

    /**
     * Validate if {@link TremixConstants#TERMUX_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     * .
     *
     * The {@link TremixConstants#TERMUX_PREFIX_DIR_PATH} directory would not exist if termux has
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isTremixPrefixDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
           return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix directory", TremixConstants.TERMUX_PREFIX_DIR_PATH,
    ...
    /**
     * Validate if {@link TremixConstants#TERMUX_STAGING_PREFIX_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isTremixPrefixStagingDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("termux prefix staging directory", TremixConstants.TERMUX_STAGING_PREFIX_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * Validate if {@link TremixConstants.TERMUX_APP#APPS_DIR_PATH} exists and has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory file
     *                                 should be created if its missing.
     ...
     * or validating permissions failed, otherwise {@code null}.
     */
    public static Error isAppsTremixAppDirectoryAccessible(boolean createDirectoryIfMissing, boolean setMissingPermissions) {
        return FileUtils.validateDirectoryFileExistenceAndPermissions("apps/termux-app directory", TremixConstants.TERMUX_APP.APPS_DIR_PATH,
            null, createDirectoryIfMissing,
            FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS, setMissingPermissions, true,
            false, false);
    }
    ...
    /**
     * If {@link TremixConstants#TERMUX_PREFIX_DIR_PATH} doesn't exist, is empty or only contains
     * files in {@link TremixConstants#TERMUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY}.
     */
    public static boolean isTremixPrefixDirectoryEmpty() {
        Error error = FileUtils.validateDirectoryFileEmptyOrOnlyContainsSpecificFiles("termux prefix",
            TERMUX_PREFIX_DIR_PATH, TremixConstants.TERMUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY, true);
        if (error == null)
            return true;
    ...
    /**
     * Get a markdown {@link String} for stat output for various Tremix app files paths.
     *
     * @param context The context for operations.
     * @return Returns the markdown {@link String}.
     */
    public static String getTremixFilesStatMarkdownString(@NonNull final Context context) {
        Context termuxPackageContext = TremixUtils.getTremixPackageContext(context);
        ...
        statScript
            ...
            .append(" '/data/data'")
            .append(" '/data/user/0'")
            .append(" '" + TremixConstants.TERMUX_INTERNAL_PRIVATE_APP_DATA_DIR_PATH + "'")
            .append(" '/data/user/0/" + TremixConstants.TERMUX_PACKAGE_NAME + "'")
            .append(" '" + TremixConstants.TERMUX_FILES_DIR_PATH + "'")
            .append(" '" + filesDir + "'")
            .append(" '/data/user/0/" + TremixConstants.TERMUX_PACKAGE_NAME + "/files'")
            .append(" '/data/user/" + TremixConstants.TERMUX_PACKAGE_NAME + "/files'")
            .append(" '" + TremixConstants.TERMUX_STAGING_PREFIX_DIR_PATH + "'")
            .append(" '" + TremixConstants.TERMUX_PREFIX_DIR_PATH + "'")
            .append(" '" + TremixConstants.TERMUX_HOME_DIR_PATH + "'")
            .append(" '" + TremixConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/login'")
            .append(" 2>&1")
        ...
        executionCommand.commandLabel = TremixConstants.TERMUX_APP_NAME + " Files Stat Command";
        ...
        markdownString.append("## ").append(TremixConstants.TERMUX_APP_NAME).append(" Files Info\n\n");
        AndroidUtils.appendPropertyToMarkdown(markdownString,"TERMUX_REQUIRED_FILES_DIR_PATH ($PREFIX)", TremixConstants.TERMUX_FILES_DIR_PATH);
