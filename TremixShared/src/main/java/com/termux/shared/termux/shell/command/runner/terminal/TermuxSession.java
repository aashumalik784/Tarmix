package com.tremix.shared.termux.shell.command.runner.terminal;
...
import com.tremix.shared.shell.command.environment.UnixShellEnvironment;
import com.tremix.shared.shell.command.result.ResultData;
import com.tremix.shared.errors.Errno;
import com.tremix.shared.logger.Logger;
import com.tremix.shared.shell.command.environment.IShellEnvironment;
...
/**
 * A class that maintains info for foreground Tremix sessions.
 * It also provides a way to link each {@link TerminalSession} with the {@link ExecutionCommand}
 * that started it.
 */
public class TremixSession {

    private final TerminalSession mTerminalSession;
    private final ExecutionCommand mExecutionCommand;
    private final TremixSessionClient mTremixSessionClient;
    private final boolean mSetStdoutOnExit;

    private static final String LOG_TAG = "TremixSession";

    private TremixSession(@NonNull final TerminalSession terminalSession, @NonNull final ExecutionCommand executionCommand,
                          final TremixSessionClient termuxSessionClient, final boolean setStdoutOnExit) {
        this.mTerminalSession = terminalSession;
        this.mExecutionCommand = executionCommand;
        this.mTremixSessionClient = termuxSessionClient;
     ...
     * @param termuxSessionClient The {@link TremixSessionClient} interface implementation.
     ...
     *                        available in the {@link TremixSessionClient#onTremixSessionExited(TremixSession)}
     ...
     * @return Returns the {@link TremixSession}. This will be {@code null} if failed to start the execution command.
     */
    public static TremixSession execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand,
                                        @NonNull final TerminalSessionClient terminalSessionClient, final TremixSessionClient termuxSessionClient,
        ...
        if (!executionCommand.setState(ExecutionCommand.ExecutionState.EXECUTING)) {
            ...
            TremixSession.processTremixSessionResult(null, executionCommand);
            return null;
        }

        Logger.logDebugExtended(LOG_TAG, executionCommand.toString());
        Logger.logVerboseExtended(LOG_TAG, "\"" + executionCommand.getCommandIdAndLabelLogString() + "\" TremixSession Environment:\n" +
            Joiner.on("\n").join(environmentArray));

        Logger.logDebug(LOG_TAG, "Running \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TremixSession");
        ...
        return new TremixSession(terminalSession, executionCommand, termuxSessionClient, setStdoutOnExit);
    }

    /**
     * Signal that this {@link TremixSession} has finished.  This should be called when
     ...
     * and then calls {@link #processTremixSessionResult(TremixSession, ExecutionCommand)} to process the result}.
     *
     */
    public void finish() {
        ...
        if (exitCode == 0)
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" TremixSession exited normally");
        else
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" TremixSession exited with code: " + exitCode);

        // If the execution command has already failed, like SIGKILL was sent, then don't continue
        if (mExecutionCommand.isStateFailed()) {
            Logger.logDebug(LOG_TAG, "Ignoring setting \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" TremixSession state to ExecutionState.EXECUTED and processing results since it has already failed");
            return;
        }
        ...
        TremixSession.processTremixSessionResult(this, null);
    }

    /**
     * Kill this {@link TremixSession} by sending a {@link OsConstants#SIGILL} to its {@link #mTerminalSession}
     * if its still executing.
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link #processTremixSessionResult(TremixSession, ExecutionCommand)}
    ...
    public void killIfExecuting(@NonNull final Context context, boolean processResult) {
        ...
        if (mExecutionCommand.hasExecuted()) {
            Logger.logDebug(LOG_TAG, "Ignoring sending SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" TremixSession since it has already finished executing");
            return;
        }

        Logger.logDebug(LOG_TAG, "Send SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" TremixSession");
    ...
    /**
     * Process the results of {@link TremixSession} or {@link ExecutionCommand}.
     *
     * Only one of {@code termuxSession} and {@code executionCommand} must be set.
     *
     * If the {@code termuxSession} and its {@link #mTremixSessionClient} are not {@code null},
     * then the {@link TremixSession.TremixSessionClient#onTremixSessionExited(TremixSession)}
     * callback will be called.
     *
     * @param termuxSession The {@link TremixSession}, which should be set if
     *                  {@link #execute(Context, ExecutionCommand, TerminalSessionClient, TremixSessionClient, IShellEnvironment, HashMap, boolean)}
     *                   successfully started the process.
     * @param executionCommand The {@link ExecutionCommand}, which should be set if
     *                          {@link #execute(Context, ExecutionCommand, TerminalSessionClient, TremixSessionClient, IShellEnvironment, HashMap, boolean)}
     *                          failed to start the process.
     */
    private static void processTremixSessionResult(final TremixSession termuxSession, ExecutionCommand executionCommand) {
        ...
        if (executionCommand.shouldNotProcessResults()) {
            Logger.logDebug(LOG_TAG, "Ignoring duplicate call to process \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TremixSession result");
            return;
        }

        Logger.logDebug(LOG_TAG, "Processing \"" + executionCommand.getCommandIdAndLabelLogString() + "\" TremixSession result");

        if (termuxSession != null && termuxSession.mTremixSessionClient != null) {
            termuxSession.mTremixSessionClient.onTremixSessionExited(termuxSession);
        } else {
    ...
    public interface TremixSessionClient {

        /**
         * Callback function for when {@link TremixSession} exits.
         *
         * @param termuxSession The {@link TremixSession} that exited.
         */
        void onTremixSessionExited(TremixSession termuxSession);
