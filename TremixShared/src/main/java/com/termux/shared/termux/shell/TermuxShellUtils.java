package com.tremix.shared.termux.shell;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import com.tremix.shared.errors.Error;
import com.tremix.shared.file.filesystem.FileTypes;
...
public class TremixShellUtils {

    private static final String LOG_TAG = "TremixShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                            ...
                            if (c == ' ' || c == '\n') {
                                if (builder.length() == 0) {
                                    ...
                                    if (shebangExecutable.startsWith("/usr") || shebangExecutable.startsWith("/bin")) {
                                        ...
                                        interpreter = TremixConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/" + binary;
                                    }
                                    break;
                                }
                            } else {
                        ...
                        interpreter = TremixConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link TremixConstants#TERMUX_TMP_PREFIX_DIR_PATH}. */
    public static void clearTremixTMPDIR(boolean onlyIfExists) {
        ...
        if(onlyIfExists && !FileUtils.directoryFileExists(TremixConstants.TERMUX_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            ...
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(TremixConstants.TERMUX_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
